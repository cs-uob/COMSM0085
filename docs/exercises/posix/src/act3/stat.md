...
A system call is a way for a linux user or program to interact with the kernel, and there are usually at least three ways of calling each one:
  ...
  3. Use a command-line program probided by your distribution.
...
Later in the manual page, it explains the `struct stat` contents. Let's have a look at the sources directly though:
  ...
  * The definition of the `struct stat` is in another file included from `sys/stat.h`; open that in your editor too and have a look at it.
...
Let's see what's going on here:
  ...
  1. `.section .text` is an assembly directive to say _the following is code_, more precisely _it goes in the code section_.
...
You can run the program with `./dostat`, but it will simply exit with status 0. What we want to do is debug it with `gdb dostat`, then do the following:
